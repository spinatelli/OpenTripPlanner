...
public class BikePNRNodeModule implements GraphBuilderModule {
    ...
    public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
        LOG.info("Computing Bike PNR nodes for street nodes...");
        ...
        for (BikeParkVertex ts : Iterables.filter(vertices, BikeParkVertex.class)) {
            ...
            profileDijkstra.setSearchTerminationStrategy(new MultiTargetTerminationStrategy(
                    parkings));
            State initial = new State(ts, new BikeParkEdge(ts), options.getSecondsSinceEpoch(),
                    options);
            // TODO: maybe
        ...
        for (BikeParkVertex ts : Iterables.filter(vertices, BikeParkVertex.class)) {
            ...
            opt.setModes(new TraverseModeSet(TraverseMode.WALK, TraverseMode.BICYCLE));
            ...
            mmdijkstra.setSearchTerminationStrategy(new MultiTargetTerminationStrategy(
                    intersections));
            time = System.currentTimeMillis();
            State initial = new State(ts, new BikeParkEdge(ts), options.getSecondsSinceEpoch(), opt);
        ...
        LOG.info(fwd + " out of " + counter + " have Bike PNR nodes");

        LOG.info("Done computing bike parking for street nodes...");
    }
