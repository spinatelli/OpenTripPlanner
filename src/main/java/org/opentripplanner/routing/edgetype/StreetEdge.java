...
public class StreetEdge extends Edge implements Cloneable {
    ...
    public void computeStreetType(GenericLocation reference, double epsilon) {
        double distance = SphericalDistanceLibrary.distance(fromv.getCoordinate(),
                reference.getCoordinate());
        double distance2 = SphericalDistanceLibrary.distance(tov.getCoordinate(),
                reference.getCoordinate());
    ...
    public boolean canTurnOnto(Edge e, State state, TraverseMode mode) {
        ...
        for (TurnRestriction turnRestriction : graph.getTurnRestrictions(this)) {
            /*
